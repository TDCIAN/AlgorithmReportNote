...
4. 링크드 리스트는 원하는 인덱스에 곧바로 접근할 수 있어, 배열 리스트보다 효율적이다. <- 이거
...
답: FILO(First In, Last Out) 또는 LIFO(Last In, First Out)
