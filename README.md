...
1. 이진 탐색 트리는 자료를 입력하는 순서에 관계 없이 항상 최적의 구조를 가지게 된다. -> 이거 
2. 이진 탐색 트리는 이진 트리 중 하나로, 항상 완전 이진 트리(Complete Binary Tree)의 구조를 가진다.
3. 최적의 구조를 갖춘 이진 탐색 트리의 탐색은 O(logN)의 시간복잡도를 가진다.
...
1. 링크드 리스트에서 첫번째 위치에 자료를 삽입하는 알고리즘
2. 해쉬 테이블에서 자료를 입력하는 알고리즘
3. 힙에 자료를 하나 추가하는 알고리즘
4. 정렬된 배열에서 가장 작은 값을 반환하는 알고리즘
