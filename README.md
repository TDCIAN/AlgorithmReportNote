...
2. 분할정복 알고리즘의 하나로, 하위 문제부터 풀어나가는 bottom-up 방식이다.
...
1. 그래프는 노드 및 간선으로 표현되는 자료 구조이며, 트리의 한 종류이다.
...
3. 탐욕 알고리즘은 brute-force 방식의 접근으로, 동적 계획법보다 비효율적이다.
4. 탐욕 알고리즘은 매 step마다(매순간) 한 개의 sub-problem만 처리한다.
...
3. 일반적으로 퀵정렬의 시간복잡도는 O(NlogN)이나, pivot으로 가장 크거나 작은 수를 선택하는 최악의 경우 O(N^3)의 시간복잡도를 가지기도 한다
